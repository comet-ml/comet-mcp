...
class TestE2ETools:
    ...
    def setup_method(self):
        ...
        if not session_context.is_initialized():
            pytest.skip("Comet ML session not initialized - check API key configuration")
    ...
    def test_list_projects_e2e(self):
        ...
        if result:  # If projects exist
            ...
            print(f"First project: {project['name']} in workspace {project['workspace']}")
    ...
    def test_search_experiments_with_project_e2e(self):
        ...
        print(f"Search 'test' in project '{project_name}' found {result['count']} experiments")
    ...
    async def test_chatbot_session_info(self):
        ...
        with patch('comet_mcp.chatbot.MCPChatbot.load_config') as mock_load_config:
            ...
            with patch('comet_mcp.chatbot.MCPChatbot._connect_server'):
                chatbot = MCPChatbot("config.json", "openai/gpt-4o-mini", {}, max_rounds=2)
                ...
                mock_tool_result.content = [
                    {"type": "text", "text": '{"initialized": true, "api_status": "Connected", "user": "test-user", "error": null}'}
                ]
                ...
                response = await chatbot.chat_once("What is the current session status?")
...
class TestE2EProjectWorkflow:
    ...
    def test_error_handling_workflow(self):
        ...
        search_result = search_experiments("very-specific-term-that-should-not-exist-12345")
