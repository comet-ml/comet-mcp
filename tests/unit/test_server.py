#!/usr/bin/env python3
"""
Unit tests for comet_mcp.server module.
...
from comet_mcp.server import parse_args, app, start_sse_server
...
class TestSSEEndpoints:
    ...
    def test_messages_endpoint_invalid_json(self):
        ...
        # Send invalid JSON
        response = client.post("/messages", data="invalid json")
...
class TestServerStartup:
    ...
    @patch('comet_mcp.server.uvicorn.Server')
    @patch('comet_mcp.server.uvicorn.Config')
    async def test_start_sse_server_calls_uvicorn(self, mock_config_class, mock_server_class):
        ...
        mock_server.serve.return_value = asyncio.sleep(0.1)  # Short sleep to simulate startup
...
class TestServerIntegration:
    ...
    def test_server_module_structure(self):
        ...
        import comet_mcp.server as server_module
    ...
    @patch('comet_mcp.server.initialize_session')
    @patch('comet_mcp.server.registry')
    def test_main_function_stdio_transport(self, mock_registry, mock_init_session):
        """Test main function with stdio transport."""
        from comet_mcp.server import main
        
        # Mock the registry
        mock_registry.get_tools.return_value = []
        
        # Mock stdio_server
        with patch('comet_mcp.server.stdio_server') as mock_stdio:
            mock_read = Mock()
            mock_write = Mock()
            mock_stdio.return_value.__aenter__.return_value = (mock_read, mock_write)
            
            # Mock the server.run method
            with patch('comet_mcp.server.server.run') as mock_run:
                mock_run.return_value = asyncio.sleep(0.1)  # Short sleep to simulate
                
                # Test with stdio transport
                with patch('comet_mcp.server.parse_args') as mock_parse:
    ...
    def test_main_function_sse_transport(self, mock_registry, mock_init_session):
        ...
        with patch('comet_mcp.server.start_sse_server') as mock_sse_server:
            mock_sse_server.return_value = asyncio.sleep(0.1)  # Short sleep to simulate
...
class TestServerCommandLine:
    ...
    def test_server_help_command(self):
        ...
        result = subprocess.run(
            ['python', '-m', 'comet_mcp.server', '--help'],
            capture_output=True,
            text=True,
            timeout=10
        )
    ...
    def test_server_invalid_transport(self):
        ...
        result = subprocess.run(
            ['python', '-m', 'comet_mcp.server', '--transport', 'invalid'],
            capture_output=True,
            text=True,
            timeout=10
        )
    ...
    def test_server_stdio_transport_startup(self):
        ...
        process = subprocess.Popen(
            ['python', '-m', 'comet_mcp.server', '--transport', 'stdio'],
    ...
    def test_server_sse_transport_startup(self):
        ...
        # Start server in background
        process = subprocess.Popen(
            ['python', '-m', 'comet_mcp.server', '--transport', 'sse', '--port', '8003'],
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
            text=True
        )
