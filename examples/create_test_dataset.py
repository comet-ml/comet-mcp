...
def load_opik_dataset(dataset_path="opik_dataset.json"):
    ...
    try:
        ...
        print("Please ensure opik_dataset_complete.json exists in the examples directory")
        return None
...
def print_dataset_info(dataset):
    ...
    print(f"Experiments covered: 6 (resnet50_baseline, resnet50_dropout, efficientnet_b0, vit_base, resnet50_augmented, efficientnet_b3)")
    ...
    for item in dataset:
        ...
        elif any(word in question for word in ["compare", "better", "outperform", "superior"]):
            question_types["performance_comparison"] += 1
        elif any(word in question for word in ["training", "converge", "curve", "progression"]):
            question_types["training_analysis"] += 1
        elif any(word in question for word in ["config", "setting", "parameter", "hyperparameter"]):
